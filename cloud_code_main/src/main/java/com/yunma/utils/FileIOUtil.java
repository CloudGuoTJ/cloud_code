...
//					return deleteDirectory(deletePath);// 为目录时调用删除目录方法
	 ...
	 * 获取系统部署项目相对位置
	 *
	 * @param addPath 追加的文件位置
	 * @return 
	 * @return
	 */
	public  String getProjectHome(String addPath){	
		String path = null;
		try {
			String pathTest = getClass().getResource("/").toURI().getPath();
			path = pathTest.substring(0,
					pathTest.lastIndexOf("webapps") + 7) + addPath;
	 ...
	 * @return
	 */
	public boolean writeInFileWithDrawn(String path,String contents){
	...
	public boolean deleteDirectory(String dirPath) {// 删除目录（文件夹）以及目录下的文件
		...
		for (int i = 0; i < files.length; i++) {// 循环遍历删除文件夹下的所有文件(包括子目录)
			if (files[i].isFile()) {// 删除子文件
				...
				logger.info("删除成功:{}"+ files[i].getAbsolutePath());
				if (!flag)
					break;// 如果删除失败，则跳出
			} else {// 运用递归，删除子目录
	...
	public static boolean createDir(String destDirName) {
		...
		if (dir.exists()) {// 判断目录是否存在
			System.out.println("目录已存在,请重新选择!");
			return false;
		}
	...
	public static String createTempFile(String prefix, String suffix,
		...
		if (dirName == null) {// 目录如果为空
			...
			if (!dir.exists()) {
				// 如果目录不存在则创建目录
				if (FileIOUtil.createDir(dirName)) {
